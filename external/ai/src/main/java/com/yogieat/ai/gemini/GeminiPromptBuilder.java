...
public class GeminiPromptBuilder {
    ...
    private static final String RESTAURANT_GENERATION_TEMPLATE = String.format(
        """
        %%s 지역 기준 반경 1km 내의 실제 인기 있는 %%s %%d개를 추천해주세요.
        ...
        각 식당에 대해:
        1. 반드시 %%s 지역 기준 반경 1km 내의 실제 가게를 추천하고, 해당 지역 이름이 아닌 먼 지역은 제외해주세요.
        2. 실제로 존재하는 유명한 식당을 추천해주세요.
        3. 중복 방지: 이미 존재하는 맛집 이름 목록과
             - 지점명(예: 강남점/본점), 띄어쓰기, 특수문자, 대소문자 차이만 있는 경우도 "중복"으로 간주하고 제외하세요.
        4. rating은 3.5~5.0 사이의 적절한 평점을 부여하세요 (소수점 첫째자리까지)
        5. largeCategory는 반드시 요청한 카테고리 값과 정확히 동일해야 합니다. 절대 다른 값으로 변경하지 마세요
        6. mediumCategory는 해당 식당의 구체적인 음식 종류를 지정하세요
        7. description은 식당의 특징과 인기메뉴를 구체적으로 설명해주세요
        8. representativeReview는 긍정적이고 구체적인 경험을 담은 리뷰를 작성해주세요
    ...
    private static final String BATCH_RESTAURANT_GENERATION_TEMPLATE = String.format(
        ...
        각 지역은 반경 1km 내에서 추천해주세요.
        이미 존재하는 맛집 이름들을 제외: %%s (중복 방지용)
        ...
        각 식당에 대해:
        1. 각 지역-카테고리 별로 실제로 존재하는 유명한 식당을 추천해주세요.
        2. 반드시 해당 지역 기준 반경 1km 내의 가게만 추천하고, 범위를 벗어난 식당은 제외하세요.
        3. 중복 방지: 이미 존재하는 맛집 이름 목록과
             - 지점명(예: 강남점/본점), 띄어쓰기, 특수문자, 대소문자 차이만 있는 경우도 "중복"으로 간주하고 제외하세요.
        4. rating은 3.5~5.0 사이의 적절한 평점을 부여하세요 (소수점 첫째자리까지)
        5. largeCategory는 반드시 요청한 카테고리 값과 정확히 동일해야 합니다. 절대 다른 값으로 변경하지 마세요
