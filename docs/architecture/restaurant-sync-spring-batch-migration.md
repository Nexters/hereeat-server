...
## 목적
현재 `batch:sync`의 커스텀 워커 기반 실행 구조를 유지하면서, 필요 시 Spring Batch로 안전하게 전환할 수 있는 단계별 설계를 정의한다.
...
현재 요구사항 범위에서는 커스텀 워커로도 충분하지만, 배치 복잡도 증가 시 Spring Batch의 표준 기능(재시작, skip/retry, execution metadata) 도입 가치가 커진다.

## 도입 판단 기준
아래 조건 중 2개 이상 충족 시 Spring Batch 전환을 권장한다.
...
### 구성
- API/도메인
  - `RestaurantSyncJobService`: Job row 생성/조회
- 실행 엔진
...
### 매핑 테이블(권장)
`Spring Batch execution`과 비즈니스 Job 매핑을 위해 아래 테이블 추가를 권장한다.
...
### Listener
- `beforeJob`
  - 상태 `RUNNING`, `startedAt`, `totalCount` 초기화
- `afterJob`
  - 성공 전건: `SUCCESS`
...
완료 기준:
- 기존 custom 경로 동작 변화 없음

### Phase 1. 병행 구성
1. `BatchConfig` 추가
  - `restaurantSyncJob`, `singleStep`, `allStep`
2. `BatchLauncherService` 추가
  - `restaurantSyncJobId`를 Job parameter로 전달
3. 매핑 테이블(`t_restaurant_sync_job_execution_map`) 반영

완료 기준:
- 로컬에서 Spring Batch job 단독 실행 가능
...
완료 기준:
- API/스케줄 호출 시 Spring Batch 경로 정상 실행
- 상태 집계가 `t_restaurant_sync_job`에 정상 반영
...
### Spring Batch 장점
- 표준화된 재시작/체크포인트
- skip/retry/listener 등 운영 기능 풍부
- Job/Step 단위 확장성 우수

### Spring Batch 단점
- 초기 구성 복잡도 증가
- 학습/운영 비용 증가
- 외부 API 중심 워크로드에서는 성능 이득이 제한적일 수 있음

## 결론
현 시점은 커스텀 워커 유지가 합리적이며, 본 가이드 기반으로 엔진 플래그를 도입해 점진 전환 가능성을 열어두는 전략을 기본으로 한다.
